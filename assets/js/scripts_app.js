...
document.getElementById("uuid-button").addEventListener("click", () => {
    ...
    if (ifUUID === 0) {
        ...
        document.getElementById("uuid").innerHTML = "<input type='text' value='" + localStorage.getItem("uuid") + "' class='uuid-input' id='uuid-input' disabled='disabled' placeholder='Aucun UUID'><button class='uuid-copy-button hover-shadow' id='uuid-copy-button'>Copier</button>"
        ifUUID = 1
...
document.getElementById('discord-button').addEventListener('click', () => {
    shell.openExternal("https://discord.gg/3YMu7qf4WD")
});
...
//     util.status('XXX.XXXXXXX.XX', { port: XXXXX }) ///////////////////////////
//         .then((response) => {
